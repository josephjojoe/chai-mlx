...
:mod:`scripts.precompute_esm_mlx`).  One subprocess per target so the
MLX Metal allocator is fully reclaimed between targets -- essential on
16 GB Macs where ESM-3B + chai-mlx would otherwise compete for memory.

Each subprocess writes:

* ``<mlx-dir>/<target>/seed_<seed>/pred.model_idx_*.cif`` -- one per
  sample, in chai-lab CIF format, ready for
  ``scripts.cuda_structure_sweep`` to consume.
...
The caller drives the subprocesses via ``subprocess.run`` so a crash in
one target does not take down the whole sweep.
"""
